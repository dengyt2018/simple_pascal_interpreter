...
pub mod pascal_parser {
    ...
    impl From<TokenType> for ReservedKeywords {
        fn from(value: TokenType) -> Self {
            match value {
                ...
                _ => {
                    panic!("unsupported token type. {}", value)
                }
            }
        }
    }
    ...
    impl Lexer {
        ...
        fn _id(&mut self) -> Token {
            ...
            if let Ok(key) = ReservedKeywords::from_str(&token) {
                match key {
                    ReservedKeywords::Procedure => Token::new(Procedure, token),
    ...
    impl Statements {
        ...
        pub fn get_procedure_decl(&self) -> (Token, ASTTree) {
            if let Statements::ProcedureDecl {
                proc_name,
                block_node,
            } = self
            {
                (proc_name.clone(), block_node.take())
            } else {
    ...
    impl ASTTree {
        ...
        pub fn procedure_decl(proc_name: Token, block_node: ASTTree) -> Rc<RefCell<Self>> {
            let mut node = ASTTree::default();
    ...
    impl Parser {
        ...
        fn declarations(&mut self) -> Vec<Rc<RefCell<ASTTree>>> {
            ...
            while self.current_token.token_type == Procedure {
                self.eat(Procedure);
                let proc_name = self.current_token.clone();
                self.eat(Id);
                self.eat(Semi);
                let block_node = self.block();
                let proc_decl = ASTTree::procedure_decl(proc_name, block_node);
    ...
    impl From<TokenType> for TypeSymbol {
        fn from(value: TokenType) -> Self {
            match value {
    ...
    impl FromStr for TypeSymbol {
        ...
        fn from_str(s: &str) -> Result<Self, Self::Err> {
            match s.to_uppercase().as_str() {
                "INTEGER" => Ok(TypeSymbol::Integer),
                "REAL" => Ok(TypeSymbol::Real),
                _ => Err(()),
            }
        }
    }
    ...
    pub struct VarSymbol {
        var_name: String,
        type_name: TypeSymbol,
    }

    impl VarSymbol {
        pub fn new<S: AsRef<str>>(var_name: S, type_name: TypeSymbol) -> Self {
            VarSymbol {
                var_name: var_name.as_ref().into(),
                type_name,
            }
        }
    }
    ...
    impl Display for VarSymbol {
        fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
            write!(f, "<{}:{}>", self.var_name, self.type_name)
        }
    }

    #[derive(Debug)]
    pub struct SymbolTable {
        _symbols: HashMap<String, VarSymbol>,
    }
    ...
    impl SymbolTable {
        ...
        pub fn define(&mut self, symbol: VarSymbol) {
            let s = symbol.clone();
            if self._symbols.is_empty() {
                self._symbols = HashMap::new();
                eprintln!("Define: {}", symbol);
                self._symbols.insert(symbol.var_name, s);
            } else {
                eprintln!("Define: {}", symbol);
                self._symbols.insert(symbol.var_name, s);
            }
        ...
        pub fn lookup(&mut self, name: &str) -> Option<&VarSymbol> {
    ...
    impl Display for SymbolTable {
        fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
            let mut symbols = vec![];
            self._symbols.iter().for_each(|x| {
                symbols.push(format!("<{}:{}>", x.0.clone(), x.1.type_name));
            });
            write!(f, "Symbols: {:#?}", symbols)
        }
    }
    ...
    impl SymbolTableBuilder {
        ...
        fn visit_var_decl(&mut self, node: ASTTree) {
            let (var_node, type_node) = node.stat.get_var_decl();
            let var_name = var_node.stat.get_token().token_value;
            let type_name = TypeSymbol::from(type_node.stat.get_token().token_type);

            self.symtab.define(VarSymbol::new(var_name, type_name));
        }

        fn visit_assign(&mut self, node: ASTTree) {
            let var_name = node.left.unwrap().take().stat.get_token().token_value;
            if self.symtab.lookup(var_name.as_ref()).is_none() {
                panic!("Visit Assign var name: {} is not define.", var_name)
            }
            self.visit(node.right.unwrap().take());
        }
        ...
        fn visit_var(&mut self, node: ASTTree) {
            ...
            if self.symtab.lookup(var_name.as_ref()).is_none() {
                panic!("visit Var name: {} is not define.", var_name)
            }
        }

        pub fn _visit(&mut self, node: ASTTree) -> String {
            self.visit(node);
            self.symtab.to_string()
        }
    }
}
