...
pub mod pascal_parser {
    ...
    impl Token {
        ...
        pub fn set_pos(mut self, lexer: &Lexer) -> Self {
            self.lineno = lexer.lineno.clone();
            self.column = lexer.column.clone();
            self
        }
    ...
    impl Lexer {
        ...
        fn advance(&mut self) {
            ...
            if self.pos > (self.text.len() - 1) {
                ...
                self.current_char = self.text[self.pos].clone();
                self.column += 1;
            }
        }
        ...
        */
        fn _id(&mut self) -> Token {
            ...
            while !self.current_char.is_whitespace() && self.current_char.is_alphanumeric() {
                buffer.push(self.current_char.clone());
                self.advance();
            }
