...
#[cfg(test)]
mod tests {
    use crate::rust_parser::part17::spi17::pascal_parser::{
...
END."
    ...
    fn test_parser() {
        ...
        ",
            ...
            Interpreter::new(Parser::new(Lexer::new(input)))
        };
...
";
    ...
    fn test_symbol_not_found() {
        let path = "./src/rust_parser/part17/dupiderror.pas";
    ...
    #[test]
    fn test_proccall() {
        let path = "./src/rust_parser/part17/part17.pas";
        if let Ok(mut file) = File::open(path) {
            let mut buff = String::new();
            file.read_to_string(&mut buff).expect("read file error");
            SemanticAnalyzer::new()._visit(Parser::new(Lexer::new(buff)).parser());
