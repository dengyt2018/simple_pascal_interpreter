...
#[cfg(test)]
mod tests {
    use crate::rust_parser::part19::spi19::pascal_parser::{
...
";
    ...
    fn test_symbol_not_found() {
        let path = "./src/rust_parser/part19/dupiderror.pas";
...
";
    ...
    #[test]
    fn test_proccall() {
        let path = "./src/rust_parser/part19/part18.pas";
        if let Ok(mut file) = File::open(path) {
            let mut buff = String::new();
            file.read_to_string(&mut buff).expect("read file error");
    ...
    #[test]
    fn test_proccall2() {
        let path = "./src/rust_parser/part19/part19.pas";
        if let Ok(mut file) = File::open(path) {
            let mut buff = String::new();
            file.read_to_string(&mut buff).expect("read file error");
            let ast = Rc::new(RefCell::new(Parser::new(Lexer::new(buff.clone())).parser()));
